...
$version = defined('JUMPYDOT_VERSION') ? JUMPYDOT_VERSION : null;
...
if ($showInfo) {
    echo("<h1>" . ENGINESIS_SITE_NAME . ' ' . $serverStage . ' (' . ENGINESIS_SITE_ID . ') version ' . JUMPYDOT_VERSION . "</h1>");
...
$database = new Database(null, 'jumpydot');
if ($database) {
    $req = $database->query('SELECT count(*) from wp_users', []);
    if ($req) {
        $row = $req->fetch();
        if ($row) {
            $pageok = true;
            ...
            $pageok = false;
            if ($showInfo) {
                echo('Error in results ' . $sqlDBs[ACTIVE_DATABASE]['db'] . ' data ' . $database->getLastError($req));
            }
        }
        $req = null;
    } else {
        $pageok = false;
        if ($showInfo) {
            echo('Error in query ' . $sqlDBs[ACTIVE_DATABASE]['db'] . ' data ' . $database->getLastError(null));
        }
    }
} else {
    ...
    $pageok = false;
    if ($showInfo) {
        echo("<p>DB=" . $sqlDBs[ACTIVE_DATABASE]['db'] . "; H=" . $sqlDBs[ACTIVE_DATABASE]['host'] ." U=" . $sqlDBs[ACTIVE_DATABASE]['user'] . "</p>\n");
    }
}
