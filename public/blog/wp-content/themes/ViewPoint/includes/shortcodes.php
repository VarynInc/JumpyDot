...
function vp_one_third($atts, $content = null){
	...
	if($noright !== '')
		$class .= 'omega';
	$output = '<div class="one-third column ' . $class . '">';
	if($icon !== '')
		$output .= '<img class="serv-icon" alt="" src="' . esc_attr($icon) . '">';
	$output .= $content . '</div>';
	return $output;
}
...
function vp_skill($atts, $content = null){
	...
	else
	{
		$value = (int)$value;
		$name = esc_attr($name);
		$output = '<div class="skill-bg"><div class="skill" style="width: ' . $value . '%"></div></div>';
		$output .= '<p>' . $name . '</p>';
		return $output;
	}
}
...
function vp_slider_img($atts, $content=null) {
	...
	{
		if($url !== '') 
			$output = ' <li><a href="' . $url . '"><img alt="' . $alt . '" src="' . $thumbnail . '"></a></li>' . PHP_EOL;
		else
